...
(
    ...
    select td.wk_of_yr, td.fecha, td.día, td.tipo_día,
            t_dat.tipo_user, t_dat.id_accion, t_dat.accion, t_dat.hora,
            to_date(to_char(substr(td.fecha,1,10)||' '||t_dat.hora||':00'), 'DD/MM/YYYY hh24:mi:ss') aux,
            case
                ...
                when t_dat.tipo_user = 'USER_3' then
                    round(dbms_random.value() * 5)
            end random_min, --Número aleatorio de minutos de variabilidad
            --round(power(dbms_random.value(-2, 2), 3)) random_mult, --Multiplicador aleatorio para minutos
            round(dbms_random.value(-1, 1)) random_mult, --Multiplicador aleatorio para minutos
            case
                when t_dat.tipo_user = 'USER_1' then
                    round(dbms_random.value(0,14)) -- Se eliminará una de cada 15
                when t_dat.tipo_user = 'USER_2' then
                    round(dbms_random.value(0,7)) -- Se eliminará una de cada 8
                when t_dat.tipo_user = 'USER_3' then
                    round(dbms_random.value(0,4)) -- Se eliminará una de cada 5
...
--select * from t_aux
--Se modifica el multiplicador para evitar ceros
t_random as
(
    select ta.wk_of_yr,
           ta.día, ta.tipo_día,
           ta.tipo_user, ta.id_accion, ta.accion, ta.hora,
           ta.aux fecha,
           ta.random_min,
           case
            when ta.random_mult = 0 then
                round(0.5 * dbms_random.value(1,2))
            else
                ta.random_mult
           end random_mult,
           ta.random_elimina_fila
    from t_aux ta
),
t_final as
(
    select tr.wk_of_yr, tr.día, tr.tipo_día,
           ...
           tr.fecha fecha_base, tr.fecha + (1/1440*(tr.random_min*tr.random_mult)) fecha,
           tr.random_min
    from t_random tr
